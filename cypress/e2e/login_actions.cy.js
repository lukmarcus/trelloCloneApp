...
describe("User Sign Up", () => {
  it("Should register and stay log in", () => {
    ...
    cy.getByData("signup-email").click().type("tester@tester.com");
    cy.getByData("signup-password").click().type("test1234");
...
describe("Registered user", () => {
  it("Should log in and logout", () => {
    ...
    cy.getByData("login-menu").click();
    cy.getByData("login-email").click().type("tester@tester.com");
    cy.getByData("login-password").click().type("test1234");
    cy.getByData("login-submit").click();

    cy.loginCheck();

    cy.getByData("logged-user").click();
    cy.visit("/");
    cy.getByData("logged-user").should("not.contain", "tester@tester.com");
  });
  ...
  it("Should create board WITH login", () => {
    ...
    cy.getByData("board-title").should("be.visible");
  });
  ...
  it("Should check if the board created WITH login is NOT visible for an anonymous user", () => {
    ...
    cy.visit("/");
    cy.getByData("logged-user").should("not.contain", "tester@tester.com");
    cy.getByData("board-item").should("not.exist");
  });
  ...
  it("Should delete board created WITH login", () => {
    ...
    cy.getByData("board-title").should("exist");
    cy.getByData("board-options").click();
    cy.getByData("delete-board").click();
