...
it("creates a card", function () {
  ...
  cy.wait("@newCard").then(({ response, request }) => {
...
it("checking the card", function () {
  ...
  cy.wait("@patchCard").its("response.statusCode").should("eq", 200);
});
...
it("creates a new list", function () {
  ...
  cy.get('[data-cy="add-list-input"]').type("List 2{enter}");

  cy.wait("@listCreate").its("request.body.boardId").should("eq", this.boardId);
});
