...
it("asserting number of lists", () => {
  cy.get('[data-cy="create-list"]').type("The Second List{enter}");
...
it("deleting a list", () => {
  cy.get('[data-cy="list-options"]').eq(1).click();

  cy.get('[data-cy="delete-list"]').click();

  cy.get('[data-cy="list-options"]').eq(1).should("not.exist");
});
...
it("asserting the checked state", () => {
  ...
  cy.get('[data-cy="new-card-input"]').type("New Item on List{enter}");

  cy.get('[data-cy="card"]').should("be.visible");

  cy.get('[data-cy="card-checkbox"]').click().should("be.checked");
...
it("asserting list name", () => {
  cy.get('[data-cy="board-title"]').click().type("New Board Title{enter}");

  cy.get('[data-cy="board-title"]').should("have.value", "New Board Title");
});
...
it("assert text of created card", () => {
  ...
  cy.get('[data-cy="new-card-input"]').click().type("Second Card{enter}");

  cy.get('[data-cy="card-text"]').eq(1).should("have.text", "Second Card");
});
...
it("checking the card detail", () => {
  ...
  cy.get('[data-cy="card-detail"]').should("exist").should("be.visible");
});
