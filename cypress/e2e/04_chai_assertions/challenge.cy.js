...
it("cards contain proper text", () => {
  cy.get("[data-cy=list]").eq(0).find("[data-cy=card-text]").as("cards");

  cy.get("@cards").then((cardText) => {
...
it("cards are checked", () => {
  ...
  cy.get("@card-checkboxes").then((checkedCard) => {
...
it("number of lists and list names", () => {
  cy.get('[data-cy="list-name"]').then((listName) => {
