...
before(() => {
  ...
  cy.get('[data-cy="new-card-input"]').type("Juice{enter}");

  cy.get('[data-cy="new-card-input"]').type("Shampoo{enter}");

  cy.get('[data-cy="new-card-input"]').type("Soap{enter}");
});
...
it("check text of all cards in first list", () => {
  cy.get("[data-cy=card-text]").then((board) => {
  ...
  cy.get("[data-cy=card-text]").should((board) => {
    expect(board[0]).to.have.text("Milk");
    expect(board[1]).to.have.text("Bread");
    expect(board[2]).to.have.text("Juice");
    expect(board[3]).to.have.text("Shampoo");
